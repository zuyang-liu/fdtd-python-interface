...
gds_file_path = os.path.join(
    ...
    'mmi2x2_with_sbend.gds'
)
# path to output file
output_dir = os.path.join(
    current_directory, 'projects', 'FDTD_solvers', 'mmi2x2', 'Data', solver,
    f'sweep_resolution/res{res}_span{span}_step5'
)
# task name for Tidy3D
task_name = f'MMI_res{res}_span{span}_step5_{datetime.now().strftime("%Y%m%d%H%M%S")}'
