...
def fdtd_from_gds(parameters):
    ...
    # mode_spec = td.ModeSpec(num_modes=mode_num, target_neff=1.9, group_index_step=True)
            ...
            if ports[port_name].orientation == 180.0 or ports[port_name].orientation == 0.0:
                if flag_flux_monitor:
                    ...
                    flux_mnt = td.FluxMonitor(
                        center = (
                            ports[port_name].center[0]*um,
                            ports[port_name].center[1]*um,
                            0,
                        ),
                        size = (
                            0,
                            (ports[port_name].width + 4.0)*um,
                            2.0*um,
                        ),
                        freqs = list(freqs),
                        name = port_name+' flux',
